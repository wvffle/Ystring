...
knownRanges = set((0x4DB5, 0x9FCC, 0xD7A3,
                   0xDB7F, 0xDBFF, 0xDFFF,
                   0xF8FF, 0x2A6D6, 0x2B734,
                   0x2B81D, 0xFFFFD, 0x10FFFD))
...
def getClassTable(fileName):
    ...
    for line in open(fileName):
        parts = line.split(";")
        charClass = parts[2]
        charId = int(parts[0], 16)
        if (charId == rangeEnd + 1 or charId in knownRanges) and charClass == rangeClass:
            rangeEnd = charId
        else:
            if rangeClass:
                values.append(value(rangeEnd, rangeClass))
            if charId != rangeEnd + 1:
                values.append(value(charId - 1, "Cn"))
            rangeClass = charClass
            rangeStart = charId
            rangeEnd = charId
    if rangeClass:
        values.append(value(rangeEnd, rangeClass))
...
def getAsciiTable(fileName):
    ...
    for line in open(fileName):
        ...
        if charId >= 128:
            break
        if len(outputLine) == 8:
            result.append(", ".join(outputLine) + ",")
            outputLine = []
        outputLine.append("0x%02X" % classes[charClass])
    result.append(", ".join(outputLine))
    return result
...
def main(args):
    if len(args) != 1:
        ...
        return 1
    writeCpp(getAsciiTable(args[0]), getClassTable(args[0]))
    return 0
